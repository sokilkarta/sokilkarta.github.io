...
const SYNC_UPLOAD_INTERVAL_MS = 80;
const SYNC_ADMIN_PUSH_INTERVAL_MS = 80;
const SYNC_QUICK_PUSH_DELAY_MS = 8;
const REMOTE_LOCK_MS = 250;
const REMOTE_MARKER_SMOOTH_MS = 140;
const MAX_TRACK_POINTS = 180;
const MAX_PATH_POINTS = 120;
const PATH_DECIMATE_STEP = 3;
...
function compactPathPoints(points){
  ...
  const out = [];
  const start = Math.max(0, points.length - (MAX_PATH_POINTS * PATH_DECIMATE_STEP));
  for(let i = start; i < points.length; i += PATH_DECIMATE_STEP){
    out.push(points[i]);
  }
  const last = points[points.length - 1];
  if(out.length === 0 || out[out.length - 1] !== last) out.push(last);
  if(out.length > MAX_PATH_POINTS) return out.slice(-MAX_PATH_POINTS);
  return out;
}
...
function serializeItem(item, includeHeavy){
  const ll = item.marker.getLatLng();
  const customSpeedMps = Number(item.customSpeedMps);
  const out = {
    ...
    customSpeedMps: Number.isFinite(customSpeedMps) && customSpeedMps > 0
      ? (Math.round(customSpeedMps * 1000) / 1000)
      : null
  };
  if(item.__kuConfig) out.kuConfig = item.__kuConfig;
  const zone = getKuZoneForItem(item);
  if(zone) out.kuZone = zone;
  if(includeHeavy && Array.isArray(item.track) && item.track.length > 1){
    out.track = item.track.slice(0,MAX_TRACK_POINTS).map(p => [Math.round(p.lat*1e6)/1e6, Math.round(p.lng*1e6)/1e6]);
  }
  if(includeHeavy && Array.isArray(item.pathHistory) && item.pathHistory.length > 1){
    out.path = compactPathPoints(item.pathHistory).map(p => [Math.round(p.lat*1e6)/1e6, Math.round(p.lng*1e6)/1e6]);
  }
  return out;
}

function serializeOwnState(includeHeavy){
  ...
  window.state.items.forEach((item, id)=>{
    ...
    try{ out[id] = serializeItem(item, includeHeavy); }catch(_e){}
  });
  return out;
}

function ensureRemoteAnimLoop(){
  if(remoteAnimRaf) return;
  const step = ()=>{
    let keepRunning = false;
    remoteGhost.forEach((g)=>{
      g.markers.forEach((m)=>{
        if(!m || !m.__syncTargetLL) return;
        const cur = m.getLatLng();
        const tgt = m.__syncTargetLL;
        const dLat = tgt.lat - cur.lat;
        const dLng = tgt.lng - cur.lng;
        const d2 = (dLat * dLat) + (dLng * dLng);
        if(d2 < 1e-12){
          if(cur.lat !== tgt.lat || cur.lng !== tgt.lng) m.setLatLng([tgt.lat, tgt.lng]);
          return;
        }
        const dt = Math.max(8, Math.min(120, Number(m.__syncDtMs) || 80));
        const alpha = Math.max(0.12, Math.min(0.9, dt / REMOTE_MARKER_SMOOTH_MS));
        m.setLatLng([cur.lat + (dLat * alpha), cur.lng + (dLng * alpha)]);
        keepRunning = true;
      });
    });
    if(keepRunning){
      remoteAnimRaf = requestAnimationFrame(step);
    } else {
      remoteAnimRaf = null;
    }
  };
...
function setMarkerRemoteTarget(marker, ll, immediate){
  if(!marker || !ll) return;
  const lat = Number(ll[0]) || 0;
  const lng = Number(ll[1]) || 0;
  const now = performance.now();
  const last = Number(marker.__syncLastTs) || now;
  marker.__syncLastTs = now;
  marker.__syncDtMs = Math.max(8, Math.min(200, now - last));
  if(immediate || !marker.__syncTargetLL){
    marker.setLatLng([lat, lng]);
  }
  marker.__syncTargetLL = L.latLng(lat, lng);
  ensureRemoteAnimLoop();
}

function applyEntryToLocal(item, entry){
  if(!item || !entry || !item.marker) return;
  const nextLL = [Number(entry.lat)||0, Number(entry.lng)||0];
  if(item.__remoteLinked){
    setMarkerRemoteTarget(item.marker, nextLL, !item.__syncInited);
    item.__syncInited = true;
  } else {
    item.marker.setLatLng(nextLL);
  }
  item.headingDeg = Number(entry.headingDeg) || 0;
  item.count = Math.max(1, Number(entry.count) || 1);
  const syncedCustomSpeed = Number(entry.customSpeedMps);
  if(Number.isFinite(syncedCustomSpeed) && syncedCustomSpeed > 0){
    item.customSpeedMps = syncedCustomSpeed;
  } else {
    delete item.customSpeedMps;
  }
  ...
  if(Object.prototype.hasOwnProperty.call(entry, 'track')){
    if(Array.isArray(entry.track) && entry.track.length > 1){
      item.track = entry.track.map(p => L.latLng(Number(p[0])||0, Number(p[1])||0));
      if(isTrackUiOn()){
        const pts = item.track.map(p => [p.lat, p.lng]);
        if(item.__remoteTrackLine){
          item.__remoteTrackLine.setLatLngs(pts);
        } else {
          item.__remoteTrackLine = L.polyline(pts, { color:'#22c55e', weight:2, dashArray:'4,6', opacity:0.8 }).addTo(map);
        }
      } else if(item.__remoteTrackLine){
        try{ map.removeLayer(item.__remoteTrackLine); }catch(_e){}
        item.__remoteTrackLine = null;
      }
    } else if(item.__remoteTrackLine){
      try{ map.removeLayer(item.__remoteTrackLine); }catch(_e){}
      item.__remoteTrackLine = null;
...
function removeLocalItem(item){
  ...
  try{ if(item.__remoteTrackLine) map.removeLayer(item.__remoteTrackLine); }catch(_e){}
  clearKuZoneForItem(item);
...
function pushAdminLinked(){
  ...
  const now = Date.now();
  const includeHeavy = (now - myLastHeavyTs) >= 450;
  if(includeHeavy) myLastHeavyTs = now;
  adminLinked.forEach((item)=>{
    if(!item || !item.__remoteOwnerUid || !item.__remoteItemId) return;
    let payload = null;
    try{ payload = serializeItem(item, includeHeavy); }catch(_e){}
    if(!payload) return;
    const sig = JSON.stringify(payload);
    if(sig === item.__remoteLastSentSig) return;
    item.__remoteLastSentSig = sig;
    item.__remoteLockUntil = Date.now() + REMOTE_LOCK_MS;
...
function uploadOwn(force){
  ...
  const now = Date.now();
  const includeHeavy = !!force || ((now - myLastHeavyTs) >= 450);
  if(includeHeavy) myLastHeavyTs = now;
  const items = serializeOwnState(includeHeavy);
...
function queueOwnUpload(){
  ...
  shareQuickTimer = setTimeout(()=>uploadOwn(true), SYNC_QUICK_PUSH_DELAY_MS);
}
...
function renderGhostUser(uid, user){
  ...
  Object.entries(items).forEach(([id, entry])=>{
    ...
    if(g.markers.has(id)){
      ...
      setMarkerRemoteTarget(m, ll, false);
      m.setIcon(ico);
      ...
      m.__syncTargetLL = L.latLng(ll[0], ll[1]);
      m.__syncLastTs = performance.now();
      m.__syncDtMs = 80;
      g.markers.set(id, m);
    }
    if(Object.prototype.hasOwnProperty.call(entry, 'path')){
    ...
    if(Object.prototype.hasOwnProperty.call(entry, 'track')){
      if(isTrackUiOn() && Array.isArray(entry.track) && entry.track.length > 1){
        ...
        else g.lines.set(idt, L.polyline(pts, { color:'#22c55e', weight:2, dashArray:'4,6', opacity:0.8 }).addTo(map));
      } else {
        const idt = `t_${id}`;
  ...
  if(isAdmin){
    ...
    remoteSnapshot.forEach((user, uid)=>{
      ...
      Object.entries(items).forEach(([itemId, entry])=>{
        const k = key(uid, itemId);
        incomingKeys.add(k);
        upsertAdminLinked(uid, itemId, entry || {});
      });
    });
    adminLinked.forEach((item, k)=>{
      if(incomingKeys.has(k)) return;
      removeLocalItem(item);
      adminLinked.delete(k);
    });
  } else {
    ...
    remoteGhost.forEach((_g, uid)=>{
      if(aliveUsers.has(uid)) return;
      clearGhost(uid);
    });
  }
...
function startSharing(){
  ...
  uploadTimer = setInterval(()=>uploadOwn(false), SYNC_UPLOAD_INTERVAL_MS);
...
function startWatching(){
  ...
  adminPushTimer = setInterval(pushAdminLinked, SYNC_ADMIN_PUSH_INTERVAL_MS);
