...
async function onFirebaseReady(){
  try{
    // Optional transport tuning; never fail init if SDK internals are sealed.
    try{
      if(firebase && firebase.database && firebase.database.INTERNAL){
        if(Object.prototype.hasOwnProperty.call(firebase.database.INTERNAL, 'forceLongPolling')){
          firebase.database.INTERNAL.forceLongPolling = true;
        }
        if(Object.prototype.hasOwnProperty.call(firebase.database.INTERNAL, 'forceWebSockets')){
    ...
    db=firebase.database(resolvedDbUrl);
